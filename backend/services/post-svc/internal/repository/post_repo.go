...
func (r *postRepo) Create(ctx context.Context, userID, content, mediaURL string) (*model.Post, error) {
	...
	err := r.pool.QueryRow(ctx,
		`INSERT INTO posts (user_id, content, media_url)
		 VALUES ($1, $2, $3)
		 RETURNING id, user_id, content, media_url, likes_count, comments_count, created_at, updated_at`,
		userID, content, mediaURL,
	).Scan(&p.ID, &p.UserID, &p.Content, &p.MediaURL, &p.LikesCount, &p.CommentsCount, &p.CreatedAt, &p.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return p, nil
...
func (r *postRepo) GetByID(ctx context.Context, id string) (*model.Post, error) {
	p := &model.Post{}
	err := r.pool.QueryRow(ctx,
		`SELECT id, user_id, content, media_url, likes_count, comments_count, created_at, updated_at
...
func (r *postRepo) GetFeed(ctx context.Context, userID, cursor string, limit int) ([]model.Post, string, bool, error) {
	...
	q := `SELECT p.id, p.user_id, p.content, p.media_url, p.likes_count, p.comments_count, p.created_at, p.updated_at
		  FROM posts p
...
func (r *postRepo) GetByUser(ctx context.Context, userID, cursor string, limit int) ([]model.Post, string, bool, error) {
	...
	args := []any{userID, limit + 1}
	q := `SELECT id, user_id, content, media_url, likes_count, comments_count, created_at, updated_at
		  FROM posts
...
func (r *postRepo) queryPosts(ctx context.Context, query string, args []any, limit int) ([]model.Post, string, bool, error) {
	...
	for rows.Next() {
		var p model.Post
		if err := rows.Scan(&p.ID, &p.UserID, &p.Content, &p.MediaURL, &p.LikesCount, &p.CommentsCount, &p.CreatedAt, &p.UpdatedAt); err != nil {
			return nil, "", false, err
		}
